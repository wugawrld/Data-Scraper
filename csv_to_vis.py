...
# Creating variable to read the selected CSV.
df = pd.read_csv('music.csv')

# Appending additonal function to variable to drop rows (ignore essentially) that are missing integer values, as in this case we only want integer values. 
df.dropna(subset=['Daily Streams'], inplace=True)
...
# Creating variables that will correspond with the selected number of songs and their respective labels. 
...
# Adjusting the aesthetic of the labels (font size, type, etc.) 
...
# Assigning the numerical value to each plot (annotating the data).
